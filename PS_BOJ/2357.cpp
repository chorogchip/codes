...
int N, NN, MM, M[401010], m[401010], A, B;

int queryM(int i, int l, int r, int s, int e) {
    if (e < l || r < s) return 1;
    if (l <= s && e <= r) return M[i];
    int md = s + e >> 1;
    return max(queryM(i<<1, l, r, s, md), queryM((i<<1)+1, l, r, md+1, e));
}
int querym(int i, int l, int r, int s, int e) {
...
int main() { FAST
    ...
    for (int i = NN; i != N + NN; ++i) cin >> M[i], m[i] = M[i];
    for (int i = NN - 1; i; --i) {
    ...
    fore (i, MM) {
        cin >> A >> B;
        cout << querym(1, A, B, 1, NN) << ' ' << queryM(1, A, B, 1, NN) << endl;
    }
}
